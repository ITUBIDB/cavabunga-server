        ...
        if(!this.getProperties().isEmpty()){
            for(Property p : this.getProperties()){
                ...
                try{
                    ...
                    throw new Validation("Todo component property validation failed: " + p.getValue());
                }
            }

            if(!(isDtstampExists && isUidExists)){
                throw new Validation("Todo component must have DTSTAMP and UID properties");
            }
        }
        ...
        if(!this.getComponents().isEmpty()){
            for(Component c: this.getComponents()){
                if(!(c instanceof Alarm)){
                    throw new Validation("Todo component must have component other then Alarm");
                }

                try{
                    c.validate();
                }catch (Exception e){
                    throw new Validation("Todo sub-component validation failed");
                }
            }
        }
    }
